...
def test_check_me1(df_in: pd.DataFrame, df_out: pd.DataFrame,
    ...
    pd.testing.assert_frame_equal(editorial_check._check_me1(input, tag, pt), output)
...
def test_check_me2(df_in: pd.DataFrame, df_out: pd.DataFrame,
    ...
    tag = semtag(len(input))
    pt = (input.loc[:, "acceptabilityId"] == "PREFERRED")
    pd.testing.assert_frame_equal(editorial_check._check_me2(input, tag, pt), output)
...
def test_check_me3(df_in: pd.DataFrame, df_out: pd.DataFrame,
    ...
    tag = semtag(len(input))
    pt = (input.loc[:, "acceptabilityId"] == "PREFERRED")
    pd.testing.assert_frame_equal(editorial_check._check_me3(input, tag, pt), output)
...
def test_check_me4(df_in: pd.DataFrame, df_out: pd.DataFrame,
    ...
    tag = semtag(len(input))
    pt = (input.loc[:, "acceptabilityId"] == "PREFERRED")
    pd.testing.assert_frame_equal(editorial_check._check_me4(input, tag, pt), output)
