...
@pytest.mark.parametrize("df_in, df_out", [("null", "null"), ("co_pa", "pa5")])
def test_check_pa5(df_in: pd.DataFrame, df_out: pd.DataFrame,
                   request: pytest.FixtureRequest) -> None:
    input = request.getfixturevalue(df_in)
    output = request.getfixturevalue(df_out)
    pd.testing.assert_frame_equal(editorial_check._check_pa5(input), output)
